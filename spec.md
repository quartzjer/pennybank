...
A single `PoWS` starts with an initial random secret bitstring (correctly sized for the current difficulty) at sequence 0 (called the `head`), and is recursively hashed-and-resized for each satoshi (last one is called the `tail`).
...
Each `PoWS` has a key of the `ripemd160(sha256(sha256(sha256(head),head+1),head+2),tail)` which is a unique verifiable public identifier for it.  The value is the `sha256(tail)` to identify the final value in the sequence.
